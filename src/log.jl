...
# coefficients from: lib/msun/src/e_log.c
@inline function _log{T}(x2::T)
    x4 = x2*x2
    p1 = @horner_oftype(x4, 6.666666666666735130e-1, 
        2.857142874366239149e-1, 1.818357216161805012e-1, 1.479819860511658591e-1)
    p2 = @horner_oftype(x4, 3.999999999940941908e-1,
        2.222219843214978396e-1, 1.531383769920937332e-1)
    return x2*p1 + x4*p2
end
...
@inline function _log{T<:SmallFloat}(x2::T)
    x4 = x2*x2
    p1 = @horner_oftype(x4, 0.66666662693023681640625,
        0.281779825687408447265625, -16.595066070556640625, -3686.712890625)
    p2 = @horner_oftype(x4, 0.4000208377838134765625,
        0.57878053188323974609375, 395.41497802734375)
    return x2*p1 + x4*p2
end
